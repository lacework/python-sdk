...
To read more about generating Lacework API keys read the Lacework docs
`here <https://docs.lacework.net/api/api-access-keys-and-tokens>`_.

If you aren't sure what a Lacework organization is you can read `here <https://docs.lacework.net/console/organization-overview>`_.
...
(if your key comes from a Lacework organization the the ``account`` property represents the
...
process it will create a file in your home directory called ``.lacework.toml`` which will contain
one or more sections containing Lacework credentials, with each section representing creds for a
single Lacework account.

If you have this file in place and you have not specified credentials using any other method then
the Python SDK willuse the credentials stored there to access the Lacework API endpoints. Note that by
